...
class ContextBuilder:
    ...
    def build_warmup(self, session: str, history_batch) -> list:
        ...
        system = (
            ...
            "2. 格式规范（务必遵守）：\n"
            "   - op=add 时记忆内容放 **content 属性**："
            "<tool name=\"memory\" op=\"add\" scope=\"...\" content=\"内容\"/>（不要用 value，不要放标签体里）\n"
            "   - 记忆**要具体**：含人物、事件、偏好、关系等可复用细节，"
            "一句话说不清的可以写 2~3 句\n"
            "3. 提取准则（scope 与粒度）：\n"
            "   - **某个人**的偏好/经历/身份 → op=add scope=user，content 里写明是谁\n"
            "   - 群聊的约定/氛围/固定梗 → op=add scope=session\n"
            "   - 跨会话通用的事实/重要背景 → op=add scope=global\n"
            "   - 同一个人多个称呼 → op=alias（user=主称呼, alias=别的称呼）\n"
            "4. **宁可少记，不记垃圾**：拿不准就不记；不确定是同一个人就别登记别名\n"
            "5. 一次可输出多个 memory 块，按顺序执行"
        )
        user = (
            f"【目标会话】{session}\n"
